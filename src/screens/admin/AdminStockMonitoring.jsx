...
export default function AdminStockMonitoring({ initialAction = null, onActionHandled }) {
  ...
  return (
    <div className="invWrap">
      ...
      {modal === "delete" && selectedItem && <div className="invModalOverlay"><div className="invModalCard deleteMode"><button className="invModalClose" type="button" onClick={() => setModal(null)}>x</button><div className="invModalTitle">Delete Stock Monitoring Item</div><p className="usersConfirmText">Delete {selectedItem.name}? This action cannot be undone.</p><div className="invModalActions"><button className="invTextBtn" type="button" onClick={() => setModal(null)}>Cancel</button><button className="invPrimaryBtn" type="button" onClick={() => { deleteStockMonitoringItem(selectedItem.id); setPage(1); setModal(null); }}>Delete</button></div></div></div>}
